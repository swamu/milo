...
function createVariant(manifest, manifestParameter, variant, isChecked) {
  const checked = {
    attribute: '',
    class: '',
  };
  ...
  const variantName = variant === 'default' ? 'Default (control)' : variant;
  return `<div>
    <input type="radio" name="${manifest.manifest}" value="${variant}" id="${manifest.manifest}--${variant}" ${checked.attribute}>
    <label for="${manifest.manifest}--${variant}" ${checked.class}>${variantName}</label>
  </div>`;
}
function createPreviewPill(manifests) {
  ...
  manifests.forEach((manifest) => {
    ...
    variantNames.forEach((variant) => {
      const isChecked = variant === manifest.selectedVariantName;
      radio += createVariant(manifest, manifestParameter, variant, isChecked);
    });
    const isChecked = !manifest.variantNames.includes(manifest.selectedVariantName);
    radio += createVariant(manifest, manifestParameter, 'default', isChecked);
