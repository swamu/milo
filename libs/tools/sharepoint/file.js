...
// ({ baseUri, site } = await getMSALConfig());
...
export async function copyItem(id, parentReference, folder, name) {
...
export async function moveFile(id, parentReference, name) {
...
export async function deleteFile({ path, item }) {
...
export async function getUploadSession(folder, name, fileSize) {
...
export async function uploadFile(url, blob) {
...
export async function breakLock(dest, destItem, size) {
 ...
 */
export async function uploadAttempt(dest, destItem, uploadUrl, blob) {
  let count = 1;
  let uploadItem;
  let url = uploadUrl;
